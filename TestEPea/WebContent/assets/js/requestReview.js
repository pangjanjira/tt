...
var viewFuncCode = REVIEW_VIEW;
var modifyFuncCode = REVIEW_MODIFY;
...
function showDetail(id) {
    ...
    toggleValidation();
    $.ajax({
        url: "RequestReviewRequestControl",
        data: {
...
$("#validateBtn").click(function() {
    if (($("#iAlsAccount").val() == "") && ($("#iReviewStatus").val() === "AP")) {
        alert("Please select account for review.");
    } else {
    	if($("#iReviewReason").val() == "") {
    		alert("Please fill in review reason.");
    	} else {
    		$("#approveForm").submit();
    	}     
    }
...
$("#approveForm").submit(function(e) {
    ...
    $.ajax({
        ...
        data: {
            ...
            "iReviewStatus": $("#iReviewStatus").val(),
            "iReviewReason": $("#iReviewReason").val(),
            "iReviewBy": $("#iReviewBy").val(),
    	...
    	if ($("#iReviewStatus").val() === "AP") {
            alertModal("MESSAGE", "The request is reviewed and sent for approval (propose for approve).", "<button type='button' class='btn btn-info btn-sm' data-dismiss='modal'>OK</button>");
        } else {
            alertModal("MESSAGE", "The request is reviewed and sent for approval (propose for reject).", "<button type='button' class='btn btn-info btn-sm' data-dismiss='modal'>OK</button>");
        }
...
function fileDownload(filename) {
    ...
    var path = curLocation.replace("request_review.jsp", "");
